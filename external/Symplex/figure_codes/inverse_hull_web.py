...
class IHWPlotter:
    ...
    def _plot_poly(self):
        ...
        return in_poly_points, c_list, n_gon_verts, nverts
    ...
    def plot_color_polygon(self, fig, ax):
        ...
        in_poly_points, c_list, n_gon_verts, nverts = self._plot_poly()
        ...
        #     if nverts == 2:
        #         vert[1] += abs(r)
        #     ax.text(vert[0], vert[1], self.elements[i], fontsize=10, color='k', va='center', ha='center')
        ax.set_xlim(-1, -0.5)
    ...
    def plot_entry(self, ax, entry):
        ...
        # if len(entry.composition.elements) > 1:
        #     if entry.annotate_str in ['HfMoNbTi', 'MoNbTi', 'HfZr']:
        ...
        #                     fontsize=10, color=entry.textcolor, alpha=1)
        self.plot_decomp(ax, entry)

    def plot(self, fig=None, ax=None, plot_color_polygon=True, fontsize = 10):
        ...
        # ax.set_title(self.target_comp_str, fontsize=fontsize)
        ax.set_xlabel('$ E_{IH} $ (eV/atom)', fontsize=fontsize)
        ax.set_ylabel('$ E_{f} $ (eV/atom)', fontsize=fontsize)
